...
export function Profile() {
  ...
  // Load real follower + following counts from Supabase. Re-fetch when demo
  // toggles or when any follow row changes in the DB so the numbers update live.
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
      ...
      if (!me) return;
      const c = await getFollowCounts(me);
